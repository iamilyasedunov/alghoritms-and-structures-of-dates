...
void insertin_q(string Next){
    ...
    int y = int(ind_3), x = ind_%3;
    check(Next, Next, -1);
    if ((0=x-1)&&(x-1=2)){
        N_next = Next;
        swap(N_next[(x-1)+y3], N_next[ind_]);
...
void decode(string way,string pattern){
    for (int i = 0; i  way.size(); i++){
        ...
        int y = int(ind_3), x = ind_%3;
        if (way[i] == 'L'){
            swap(pattern[(x-1)+y3], pattern[ind_]);
        }
        if (way[i] == 'R'){
            swap(pattern[(x+1)+y3], pattern[ind_]);
        }
...
int main()
{
    ...
    while(!q.empty()){
        ...
        if (pattern == temp){
            int n = q.size();
            coutheeeere pattern way[pattern]endl;
            coutfinal Q iteration iterendlendl;
